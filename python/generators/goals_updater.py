"""
Goals updater — suggests updates to PG objectives after a session via MiniMax API.

Post-session content generation (M4-T7).
Uses the MiniMax 2.7 API to generate goal update suggestions from the PG's perspective,
...
from generators.prompts import CharacterContext, build_goals_update_prompt
...
def update_goals(
    ...
    *,
    ...
    """
    Generate goal update suggestions for a PG after a session.

    Steps:
      1. Load character and session rows from the DB.
      2. Build a MiniMax prompt using build_goals_update_prompt().
    ...
    Args:
        ...
        completed_objectives: String describing objectives completed during the session.
    ...
    prompt = build_goals_update_prompt(
        character=character,
    ...
    record = GeneratedContent(
        ...
        context=f"Goals update for session '{sess_row.get('title', '')}' ({session_id})",
        generated_at=now,
    )
