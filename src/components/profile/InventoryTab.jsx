...
export default function InventoryTab({ inventory }) {
  ...
  const totalWeight = inventory.reduce((sum, item) => sum + (item.weight * item.quantity), 0).toFixed(1);
  const equippedWeight = inventory.filter(i => i.equipped).reduce((sum, item) => sum + item.weight * item.quantity, 0).toFixed(1);
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-sm text-text-muted">
          <span className="font-medium text-text-secondary">{inventory.length}</span> oggetti •
      ...
      <div className="card p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map(item => (
              <tr key={item.id} className="border-b border-border-primary hover:bg-bg-tertiary transition-colors">
